...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ["ROOT"]     # stack has only the ROOT
        self.buffer = sentence[:] # buffer has all the words in a sentence except ROOT 
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:]   # List of partialParse objects (shallow copy)
    while len(unfinished_parses) > 0:
        transitions = model.predict(unfinished_parses[:batch_size])
        for i, transition in enumerate(transitions):
            unfinished_parses[i].parse_step(transition)
        unfinished_parses[:] = [unfinished_parse for unfinished_parse in unfinished_parses if(len(unfinished_parse.stack) != 1 or len(unfinished_parse.buffer) != 0)]
