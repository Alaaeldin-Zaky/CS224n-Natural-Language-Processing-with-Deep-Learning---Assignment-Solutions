...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.Tensor(w.shape[0], self.n_features * self.embed_size)
        for batch_no in range(w.shape[0]):
            x[batch_no] = torch.index_select(self.embeddings, 0, w[batch_no]).view(self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        hidden_layer_drop = self.dropout(hidden_layer)
        logits = torch.matmul(hidden_layer_drop, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
